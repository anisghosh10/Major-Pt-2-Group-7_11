...
#df['Age_cate2']=pd.factorize(df.Age_cate)[0]       #Replacing them
...
#Enter if the person had any depression. Enter 0 for no, 1 for yes
Label(root,text="Enter 0 for no Depression or 1 for yes  : ", font=('Helvetica',10,'bold'),bg="light green",relief="solid").place(x=40,y=180)

#Enter if the person attempted suicide or not
Label(root,text="Enter 0 if person attempted suicide or 1 if no: ", font=('Helvetica',10,'bold'),bg="light green",relief="solid").place(x=40,y=230)

#Enter the Culture shock score
Label(root,text="Enter the culture shock score: ", font=('Helvetica',10,'bold'),bg="light green",relief="solid").place(x=40,y=280)

#Enter the willingness to take Relative's help
Label(root,text="Enter the willingness to relative score: ", font=('Helvetica',10,'bold'),bg="light green",relief="solid").place(x=40,y=330)
...
Entry(root,text=dep,width=25).place(x=400,y=180)
Entry(root,text=sui,width=25).place(x=400,y=230)
Entry(root,text=acs,width=25).place(x=400,y=280)
Entry(root,text=rel,width=25).place(x=400,y=330)
Entry(root,text=cat,width=25).place(x=400,y=380)

def model():
    #Defining the features and labels
    x1 = df.iloc[:, 5:6] #Age_cate feature
    x2 = df.iloc[:, 50:51] #Depression feature
    x3 = df.iloc[:, 51:52] #Suicide feature
    x4=df.iloc[:,24:25] #ACS feature
    x5=df.iloc[:,31:32] #Relative feature
    ...
    x_test=[int(dep.get()),int(sui.get()),int(acs.get()),int(rel.get()),int(cat.get())] #Getting the values
    y_pred=model.predict([x_test,]) #Getting the predicted result
    s="Major Depressive Disorder" #2 for Major Depression
    if(str(list(y_pred)[0])=='1'): #1 for Depression
        s="Other Depressive Disorder"
    elif(str(list(y_pred)[0])=='0'): #0 for No Depression
        s="No Depression"
